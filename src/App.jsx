 ...
 * GitHub Export Version - Fixed for Vercel Deployment
 */
...
// --- CORE INTELLIGENCE API (FIXED FOR SERVERLESS) ---
...
const InputGroup = ({ label, icon: Icon, children }) => (
  <div className="space-y-3">
    <div className="flex items-center gap-3 px-2">
      ...
      <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">{label}</label>
    </div>
    {children}
  </div>
);
...
const NavItem = ({ icon: Icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-5 rounded-2xl transition-all group ${active ? 'bg-zinc-800 text-white shadow-xl' : 'text-zinc-600 hover:text-zinc-400 hover:bg-zinc-900/50'}`}>
...
const EmptyState = ({ icon: Icon, title, subtitle }) => (
  <div className="flex flex-col items-center justify-center py-32 text-center space-y-8 opacity-40">
    <Icon size={120} className="text-zinc-800" />
    <div className="space-y-3">
      <h3 className="text-3xl font-black uppercase tracking-tight text-zinc-700 italic">{title}</h3>
      <p className="text-zinc-600 text-sm max-w-md italic font-medium">{subtitle}</p>
    </div>
  </div>
);
...
export default function App() {
  ...
  const handleRecs = async () => {
    ...
    try {
      ...
      setError("Node Synthesis Failed"); 
      console.error(e);
    }
    setLoading(false);
  };
  ...
  const handleComp = async () => {
    ...
    try {
      ...
      setError("Head-to-Head Audit Failed"); 
      console.error(e);
    }
    setLoading(false);
  };
  ...
  const handleAna = async () => {
    ...
    try {
      ...
      setError("Deep Node Audit Failed"); 
      console.error(e);
    }
    setLoading(false);
  };
  ...
  const handleAdvisor = async () => {
    ...
    try {
      ...
      setAdvisorChat(p => [...p, { role: 'ai', text: "Inference Error." }]); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 flex flex-col md:flex-row font-sans selection:bg-blue-500/30">
          ...
          {activeTab === 'pathfinder' && (
            <div className="max-w-6xl mx-auto space-y-16 animate-in fade-in slide-in-from-right-8 duration-700">
              <div className="flex flex-col md:flex-row items-center gap-8 bg-zinc-900/50 p-4 rounded-[3.5rem] border border-zinc-800 focus-within:border-zinc-600 transition-all shadow-2xl">
                ...
                <button onClick={handleAna} disabled={loading} className="w-full md:w-auto px-16 py-10 bg-white text-black rounded-[2.5rem] hover:bg-zinc-200 transition-all flex items-center justify-center gap-4 group shadow-2xl disabled:opacity-30">
              ...
              {analysis ? (
                <div className="space-y-20 pb-32">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
                    ...
                    <div className="space-y-12">
                      ...
                      <div className="p-10 bg-zinc-900/20 border border-zinc-800/60 rounded-[3rem] space-y-6">
                        <h4 className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.4em] italic">Critical Node Dependencies</h4>
                        <div className="flex flex-wrap gap-3">
                          {analysis.criticalDependencies.map((dep, i) => (
                            <span key={i} className="px-5 py-3 bg-zinc-950 border border-zinc-800 rounded-2xl text-sm font-bold text-zinc-400 hover:text-white hover:border-zinc-600 transition-all cursor-default">
                              {dep}
                            </span>
                          ))}
                        </div>
                ...
                <EmptyState icon={Network} title="Awaiting Node Query" subtitle="Enter a financial node identifier to begin deep logical traversal and historical pathway synthesis." />
              )}
            </div>
          )}
        </div>
      ...
      {/* ADVISOR MODAL */}
      {isAdvisorOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-4" onClick={() => setIsAdvisorOpen(false)}>
          <div className="w-full max-w-4xl h-[80vh] bg-zinc-900 border border-zinc-800 rounded-[3rem] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-10 border-b border-zinc-800 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <MessageSquare size={24} className="text-blue-500" />
                <h3 className="text-2xl font-black italic uppercase tracking-tight text-white">Logic Advisor</h3>
              </div>
              <button onClick={() => setIsAdvisorOpen(false)} className="p-3 hover:bg-zinc-800 rounded-xl transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-6">
              {advisorChat.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full opacity-40">
                  <MessageSquare size={80} className="text-zinc-800 mb-6" />
                  <p className="text-zinc-600 text-lg italic font-medium text-center max-w-md">
                    Ask me anything about discrete logic, financial reasoning, or system states.
                  </p>
                </div>
              )}
              {advisorChat.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-6 rounded-3xl ${msg.role === 'user' ? 'bg-blue-500/10 border border-blue-500/20 text-blue-100' : 'bg-zinc-800 border border-zinc-700 text-zinc-300'}`}>
            ...
            <div className="p-10 border-t border-zinc-800">
              <div className="flex gap-4">
                <input 
                  ...
                  placeholder="Type your logical inquiry..." 
                  className="flex-1 bg-zinc-950 border border-zinc-800 rounded-2xl px-6 py-4 text-sm text-zinc-100 placeholder:text-zinc-700 focus:outline-none focus:border-zinc-600 transition-all"
                />
                <button onClick={handleAdvisor} className="px-10 py-4 bg-blue-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-400 transition-all flex items-center gap-3">
                  <ArrowRight size={18} /> Ask
                </button>
              </div>
            </div>
          </div>
